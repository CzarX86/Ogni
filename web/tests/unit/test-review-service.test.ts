...
describe('ReviewService', () => {
  ...
  describe('getProductReviews', () => {
    it('should return reviews for a product', async () => {
      ...
      mockApiClient.queryCollection.mockResolvedValue(mockReviews);

      const result = await ReviewService.getProductReviews('product1');

      expect(mockApiClient.queryCollection).toHaveBeenCalledWith(
        'reviews',
        [{ field: 'productId', operator: '==', value: 'product1' }],
        'createdAt',
        'desc',
        10
      );
    ...
    it('should handle empty results', async () => {
      mockApiClient.queryCollection.mockResolvedValue([]);
  ...
  describe('getUserReviews', () => {
    it('should return user reviews', async () => {
      ...
      expect(mockApiClient.queryCollection).toHaveBeenCalledWith(
        'reviews',
        [{ field: 'userId', operator: '==', value: 'user1' }],
        'createdAt',
        'desc',
        20
      );
  ...
  describe('canUserReviewProduct', () => {
    ...
    it('should return false if user has already reviewed the product', async () => {
      ...
      mockApiClient.queryCollection.mockResolvedValue(existingReview);
